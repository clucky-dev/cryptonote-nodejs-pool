...
var cnHashing = require('plenteum-multi-hashing');
...
//soft shell defaults
var softshellMemory = config.softshellMemory || 262144;
var softshellWindow = config.softshellWindow || 2048;
var softshellMultiplier = config.softshellMultiplier || 3;

var cryptoNight;
var cryptoNightLite;
var cryptoNightTurtle;
...
if (!cnHashing || !cnHashing[cnAlgorithm]) {
    ...
    cryptoNightTurtle = cnHashing["cryptonightturtlelite"];
    ...
    getJob: function(){
        ...
        if (!this.proxy) {
            ...
            this.cachedJob = {
                ...
                memory: softshellMemory,
                window: softshellWindow,
                multiplier: softshellMultiplier,
                id: this.id
            };
        }else {
...
function processShare(miner, job, blockTemplate, params){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        if (hard_fork_version <= 4) {
            ...
            log('info', logSystem, 'Mining pool algorithm: Cryptonight Turtle variant 1, Hard fork version: %d', [hard_fork_version]);
            hash = cryptoNightTurtle(convertedBlob, 2); //cn Turtle light 2
        } else {
            log('info', logSystem, 'Mining pool algorithm: %s variant %d, Hard fork version: %d, at height %d, memory %d, window %d and multiplier %d', [cnAlgorithm, cnVariant, hard_fork_version, blockTemplate.height, softshellMemory, softshellWindow, softshellMultiplier]);
            hash = cryptoNight(convertedBlob, cnVariant, blockTemplate.height, softshellMemory, softshellWindow, softshellMultiplier); //send height , memory, window and multiplier as aditional params to calculate scratchpad and iterations in hashing func
        }
